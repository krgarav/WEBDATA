...
const getCsvCompareData = async (req, res) => {
  ...
  try {
    ...
    const minIndex = parseInt(min), maxIndex = parseInt(max);
    ...
    for await (const row of csvStream) {
      ...
      // Capture Previous and Current Index Data
      if (rowIndex - 1 === currentIndex - 1) previousData = { ...row };
      if (rowIndex - 1 === currentIndex) filteredResult = { ...row };
      
      if (filteredResult && previousData) break; // Stop early if we have required data
    }
